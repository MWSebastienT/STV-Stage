...
class PromoController extends Controller
{
    ...
    public function addAction(Request $request)
    {
        ...
        $data = $this->container->get('back.method.actions')->formAction($request, $form, $promo, $entityName,$listPromo, $action,'Year');// true parce que j'utilise la table User pour add
        if ($data[0] == 'validate') // si on est dans la validation du formulaire
        {
            $session = new Session();
